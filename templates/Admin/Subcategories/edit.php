<div class="page-content">
	<div class="page-header">
		...
		<h1>Category<small><i class="ace-icon fa fa-angle-double-right"></i> Edit Subcategory</small>
		</h1>
	</div>
	...
	<div class="row">
		<div class="col-xs-12">
			...
			<input type="hidden" name="data[Subcategory][id]" value="<?php echo h($Subcategory['Subcategory']['id'] ?? ''); ?>" />
			<div class="form-group">
				...
				<div class="col-sm-8">
					...
					<select class="col-xs-10 col-sm-5" name="data[Subcategory][category_id]">
						<?php foreach ($category as $category_arr) { ?>
							<option value="<?php echo h($category_arr['Category']['id']); ?>"  <?php if (($category_arr['Category']['id'] ?? '') == ($Subcategory['Subcategory']['category_id'] ?? '')) { echo 'selected'; } ?>   ><?php echo h($category_arr['Category']['category_name']); ?></option>
						<?php } ?>
					</select>
				...
				<div class="form-group">
					...
					<div class="col-sm-8">
						<input type="text" name="data[Subcategory][name]" id="category_name" class="col-xs-10 col-sm-5" placeholder="Sub Category Name" value="<?php echo h($Subcategory['Subcategory']['name'] ?? ''); ?>" />
					</div>
				</div>
				<div class="form-group">
					<label for="form-field-2" class="col-sm-3 control-label no-padding-right"> Status : </label>
						<div class="col-sm-9">
						<?php
						$st = (int)($Subcategory['Subcategory']['status'] ?? 0);
						$active = $st === 1 ? 'checked' : '';
						$inactive = $st !== 1 ? 'checked' : '';
						?>
