	...
	<div class="page-content">
		<div class="page-header">
			<h1>DuroLab <?php  echo $this->Html->link('Add New',array('controller' => 'tasks','action' => 'add'),array('class'=>'btn btn-info btn-xs top-button'));  ?>
			
			<form action="" method="post" style="float:right;">
				<input type="text" name="keyowrd" required value="<?php echo $keyowrd; ?>" placeholder="Search By TaskID, title" style="width:174px !important">
				<input type="submit" name="search"  value="search" class="btn btn-info btn-xs" style="margin-top:10px" >	
				<a href="<?php echo SITEURL.'admin/tasks/index'?>" class="btn btn-warning btn-xs" style="margin-top:10px">Show All</a>
			</form>	</h1>
		</div>
	<?php if($is_access == 1){ ?>	
	<div class="row">
		...
		<div class="col-xs-12">
			...
			<form action="" class="form-horizontal" id="" method="post" accept-charset="utf-8">
				<?php if(!empty($usernewArr['User']['email']) && !empty($usernewArr['User']['phone'])){ ?>
					<p><b>Form enhaced security of your account, we have sent One-Time Password (OTP) to your mobile number <?php echo $usernewArr['User']['phone'] ?> and email Id <?php echo $usernewArr['User']['email'] ?></b></p>	
				<?php }else { ?>
					<p><b>Form enhaced security of your account, we have sent One-Time Password (OTP) to you email Id <?php echo $usernewArr['User']['email'] ?></b></p>	
				<?php  } ?>
		...
		<div class="col-xs-12">
			...
			<!-- 	<tr>
				...
				<tbody>
					...
					<td>
					<?php if($tasks['Task']['admin_id'] == 0){ echo '<span class="label label-info">'.$tasks['NappUser']['name'].' '.$tasks['NappUser']['lname'].'</span>'; }else{ echo  '<span class="label label-success">'.$tasks['User']['name'].'</span>'; } ?>
                    ...
                    <td> <?php echo date('d-M-Y',strtotime($tasks['Task']['created'])); ?></td>
					 <td class="actions">
						...
						if(($user_id == $tasks['Task']['admin_id']) && ($tasks['Task']['task_status'] < 3)){ ?>
							...
							<?php echo $this->Form->postLink(__('Complete'), array('action' => 'completetask', $tasks['Task']['task_id'],'completed'), array('class'=>'btn btn-mini btn-success'), __('Are you sure you want to delete # %s?', $tasks['Task']['task_id'])); ?>
